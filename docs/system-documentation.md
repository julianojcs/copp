...
Foto da galeria: uploadedBy, url, publicId, thumbnailUrl, title, description, location, takenAt, taggedUsers[], isPublic. Indices: createdAt desc, taggedUsers, text(location+title+description). Reacoes sao rastreadas polimorficamente em `Reaction` (`targetType: 'photo'`); o campo legado `likes[]` foi retirado na issue #12.
...
Migracao do legado `Photo.likes`: `npx tsx scripts/migrate-photo-likes-to-reactions.ts` (suporta `--dry-run` e `--cleanup` para fazer `$unset` do campo apos o backfill). Idempotente via indice unico - re-execucoes pulam duplicatas. O script le `photos.likes` via collection raw, portanto continua funcional mesmo apos a remocao do campo do schema Mongoose (issue #12). O endpoint legado `POST /api/photos/[id]/like` foi removido.
